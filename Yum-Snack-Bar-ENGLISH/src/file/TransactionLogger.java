...
public class TransactionLogger {
	...
	public static void logTransactions(List<Transaction> transactions) {
		Transaction highestTransaction = Collections.max(transactions);
		
		try (BufferedWriter bw = new BufferedWriter(new FileWriter(FILE_NAME, true))) {
			for (Transaction transaction : transactions) {
				String line = transaction.getFormattedTimestamp() 
						+ " ID=" + transaction.getID() + ", ";
				for (int i = 0; i < transaction.getItems().size(); i++) {
					line += transaction.getItems().get(i).getName();
				...
				line += "price=" + transaction.getTotalCost();
